    ...
    <div className="flex h-full">
      <aside className="hidden w-64 shrink-0 flex-col border-r border-hairline bg-surface p-4 md:flex justify-between">
        ...
        <div className="border-t border-hairline pt-4">
          <div className="flex items-center gap-3 px-2 py-2">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-amber-500/15 text-sm font-bold text-amber-600">
              {user?.name ? user.name.charAt(0) : "A"}
            </div>
            <div className="min-w-0 flex-1">
              <div className="truncate text-xs font-semibold text-ink">{user?.name || "System Administrator"}</div>
              <div className="truncate text-[11px] text-ink-muted">{user?.region || "Central HQ"}</div>
            </div>
          </div>
          <button
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="flex items-center justify-between border-b border-hairline bg-surface px-6 py-3">
          <h1 className="text-base font-semibold text-ink">System Infrastructure · Mesh Health</h1>
